...
public abstract class AbstractScreen implements Screen {
	...
	public void addFPS(boolean isFPS)
	{
		this.isFPS=isFPS;
		LabelStyle labelStyle = new LabelStyle(new BitmapFont(), Color.WHITE); // 创建一个Label样式，使用默认黑色字体
		Label label = new Label("FPS:", labelStyle); // 创建标签，显示的文字是FPS：
		label.setName("fpsLabel"); // 设置标签名称为fpsLabel
		//左上角
		label.setPosition(0, AppSettings.WORLD_HEIGHT-label.getHeight()); // 设置X,Y为0，即显示在左下角
